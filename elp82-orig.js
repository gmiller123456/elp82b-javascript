...
//Organization of the code has mostly been kept the same, see
//elp82.js for a simplified version which may be easier
//to follow.
...
class ELP82B{
    static ELP82B(tjj) {
        ...
        */
        ...
        for (let i = 0; i < 3; i++) {
            this.mainproblem(elp82data[i], t, del, dtasm, am, i + 1, r)
        }

        for (let i = 3; i < 9; i++) {
            this.tides(elp82data[i], t, del, zeta, i + 1, r);
        }

        for (let i = 9; i < 21; i++) {
            this.planetary(elp82data[i], t, del, i + 1, r);
        }
        ...
        this.convertToJ2000(r, t, w);

        return r;
    }

    static convertToJ2000(r, t, w) {
        const rad = 648000. / Math.PI;
    ...
    //Main problem.
    static mainproblem(coeficients, t, del, dtasm, am, ific, r) {
        const rad = 648000. / Math.PI;
    ...
    static planetary(coeficients, t, del, ific, r) {
        const deg = Math.PI / 180.;
        const rad = 648000. / Math.PI;
    ...
    static tides(coeficients, t, del, zeta, ific, r) {
        const iv = ((ific - 1) % 3) + 1
        for (let l = 0; l < coeficients.length; l++) {
