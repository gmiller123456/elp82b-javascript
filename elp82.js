//Greg Miller (gmiller@gregmiller.net) 2021
//Released as public domain
//Conversion of original EPP82B Fortran code to JavaScript

function ELP82B (tjj){
/*
//     Input :
//     tjj    julian date TDB (real double precision).
//
//     Output :
//     r[3]   table of rectangular coordinates (real double precision).
//            reference frame : mean dynamical ecliptic and inertial
//            equinox of J2000 (JD 2451545.0).
//            r[1] : X (kilometer).
//            r[2] : Y (kilometer).
//            r[3] : Z (kilometer).
*/

//     Initialisation.
///
   let r=[0,0,0,0];

   cpi=3.141592653589793;
   cpi2=2.*cpi;
   pis2=cpi/2.;
   rad=648000./cpi;
   deg=cpi/180.;
   c1=60.;
   c2=3600.;
   ath=384747.9806743165;
   a0=384747.9806448954;
   am=0.074801329518;
   alfa=0.002571881335;
   dtasm=2.*alfa/(3.*am);
//
//     Lunar arguments.
//
   let w=[[0],[0],[0],[0]];
   let eart=[];
   let peri=[];
   w[1][1]=(218+18/c1+59.95571/c2)*deg;
   w[2][1]=(83+21/c1+11.67475/c2)*deg;
   w[3][1]=(125+2/c1+40.39816/c2)*deg;
   eart[1]=(100+27/c1+59.22059/c2)*deg;
   peri[1]=(102+56/c1+14.42753/c2)*deg;
   w[1][2]=1732559343.73604/rad;
   w[2][2]=14643420.2632/rad;
   w[3][2]=-6967919.3622/rad;
   eart[2]=129597742.2758/rad;
   peri[2]=1161.2283/rad;
   w[1][3]=-5.8883/rad;
   w[2][3]=-38.2776/rad;
   w[3][3]=6.3622/rad;
   eart[3]=-0.0202/rad;
   peri[3]=0.5327/rad;
   w[1][4]=0.6604-2/rad;
   w[2][4]=-0.45047-1/rad;
   w[3][4]=0.7625-2/rad;
   eart[4]=0.9-5/rad;
   peri[4]=-0.138-3/rad;
   w[1][5]=-0.3169-4/rad;
   w[2][5]=0.21301-3/rad;
   w[3][5]=-0.3586-4/rad;
   eart[5]=0.15-6/rad;
   peri[5]=0.;
//
//     Planetary arguments.
//
   let p=[[],[],[],[],[],[],[],[],[]];
   
   preces=5029.0966/rad;
   p[1][1]=(252+15/c1+3.25986/c2)*deg;
   p[2][1]=(181+58/c1+47.28305/c2)*deg;
   p[3][1]=eart[1];
   p[4][1]=(355+25/c1+59.78866/c2)*deg;
   p[5][1]=(34+21/c1+5.34212/c2)*deg;
   p[6][1]=(50+4/c1+38.89694/c2)*deg;
   p[7][1]=(314+3/c1+18.01841/c2)*deg;
   p[8][1]=(304+20/c1+55.19575/c2)*deg;
   p[1][2]=538101628.68898/rad;
   p[2][2]=210664136.43355/rad;
   p[3][2]=eart[2];
   p[4][2]=68905077.59284/rad;
   p[5][2]=10925660.42861/rad;
   p[6][2]=4399609.65932/rad;
   p[7][2]=1542481.19393/rad;
   p[8][2]=786550.32074/rad;
//
//     Corrections of the constants (fit to DE200/LE200).
//
   const delnu=+0.55604/rad/w[1][2];
   const dele=+0.01789/rad;
   const delg=-0.08066/rad;
   const delnp=-0.06424/rad/w[1][2];
   const delep=-0.12879/rad;
//
//     Delaunay's arguments.
//
   let del=[[],[],[],[],[],];
   for(let i=1;i<=5;i++){
      del[1][i]=w[1][i]-eart[i];
      del[4][i]=w[1][i]-w[3][i];
      del[3][i]=w[1][i]-w[2][i];
      del[2][i]=eart[i]-peri[i];
   }
   del[1][1]=del[1][1]+cpi;

   let zeta=[];
   zeta[1]=w[1][1];
   zeta[2]=w[1][2]+preces;
//
//     Precession matrix.
//
   const p1=0.10180391e-4;
   const p2=0.47020439e-6;
   const p3=-0.5417367e-9;
   const p4=-0.2507948e-11;
   const p5=0.463486e-14;
   const q1=-0.113469002e-3;
   const q2=0.12372674e-6;
   const q3=0.1265417e-8;
   const q4=-0.1371808e-11;
   const q5=-0.320334e-14;

   ideb=1

   let t=[];
   t[1]=1.
   t[2]=(tjj-2451545.0)/36525.;
   t[3]=t[2]*t[2];
   t[4]=t[3]*t[2];
   t[5]=t[4]*t[2];

   ific=1;
   ierr=0;

//     Main problem.
// 
    for(let mainProblem=0;mainProblem<3;mainProblem++){
        coeficients=data[mainProblem];
        iv=((ific-1)%3)+1
        for(let l=0;l<coeficients.length;l++){
            ilu=coeficients[l][0];
            coef=coeficients[l][1];
            x=coef[0]
            tgv=coef[1]+dtasm*coef[5]
            if (ific==3) coef[0]=coef[0]-2.*coef[0]*delnu/3.
            x=coef[0]+tgv*(delnp-am*delnu)+coef[2]*delg+coef[3]*dele+coef[4]*delep
            y=0.
            for(let k=1;k<=5;k++){
                for(let i=1;i<=4;i++){
                    y=y+ilu[i-1]*del[i][k]*t[k];
                }
            }
            if (iv==3) y=y+pis2
            y=y%cpi2
            r[iv]=r[iv]+x*Math.sin(y);
        }
        ific=ific+1
    }
//
//     Figures - Tides - Relativity - Solar eccentricity.
//
   for(let tides=3;tides<9;tides++){
         coeficients=data[tides];
         iv=((ific-1)%3)+1
         for(let l=0;l<coeficients.length;l++){
               iz=coeficients[l][0];
               ilu=coeficients[l][1];
               pha=coeficients[l][2];
               x=coeficients[l][3];
               per=coeficients[l][4];
               
               if (ific>=7 && ific<=9) x=x*t[2]
               if (ific>=25 && ific<=27) x=x*t[2]
               if (ific>=34 && ific<=36) x=x*t[3]
               y=pha*deg
               for(let k=1;k<=2;k++){
                     y=y+iz*zeta[k]*t[k]
                     for(let i=1; i<=4; i++){
                           y=y+ilu[i-1]*del[i][k]*t[k]
                     }
               }
               y=y%cpi2
               r[iv]=r[iv]+x*Math.sin(y)
         }
         ific=ific+1
   }
//
//     Planetary perturbations.
//
      for(let planetaryPert=9;planetaryPert<21;planetaryPert++){
            coeficients=data[planetaryPert];
            iv=((ific-1)%3)+1
            for(let l=0;l<coeficients.length;l++){
                  ipla=coeficients[l][0];
                  pha=coeficients[l][1];
                  x=coeficients[l][2];
                  per=coeficients[l][3];

                  if (ific>=13 && ific<=15) x=x*t[2]
                  if (ific>=19 && ific<=21) x=x*t[2]
                  y=pha*deg

                  if (ific<16){
                        for(let k=1; k<=2;k++){
                              y=y+(ipla[8]*del[1][k]+ipla[9]*del[3][k]+ipla[10]*del[4][k])*t[k]
                              for(let i=1;i<=8;i++){
                                    y=y+ipla[i-1]*p[i][k]*t[k]
                              }
                        }
                  } else {
                        for(let k=1;k<=2;k++){
                              for(let i=1;i<=4;i++){
                                    y=y+ipla[i+7-1]*del[i][k]*t[k]
                              }
                              for(let i=1;i<=7;i++){
                                    y=y+ipla[i-1]*p[i][k]*t[k]
                              }
                        }
                  }
                  y=y%cpi2
                  r[iv]=r[iv]+x*Math.sin(y)
            }
            ific=ific+1
      }
//
//     Figures - Tides - Relativity - Solar eccentricity.
//
      for(let tides=21;tides<36;tides++){
            coeficients=data[tides];
            iv=((ific-1)%3)+1
            for(let l=0;l<coeficients.length;l++){
                  iz=coeficients[l][0];
                  ilu=coeficients[l][1];
                  pha=coeficients[l][2];
                  x=coeficients[l][3];
                  per=coeficients[l][4];

                  if (ific>=7 && ific<=9) x=x*t[2]
                  if (ific>=25 && ific<=27) x=x*t[2]
                  if (ific>=34 && ific<=36) x=x*t[3]
                  y=pha*deg
                  for(let k=1;k<=2;k++){
                        y=y+iz*zeta[k]*t[k]
                        for(let i=1; i<=4; i++){
                              y=y+ilu[i-1]*del[i][k]*t[k]
                        }
                  }
                  y=y%cpi2
                  r[iv]=r[iv]+x*Math.sin(y)
            }
            ific=ific+1
      }


//
//     Change of coordinates.
//
      r[1]=r[1]/rad+w[1][1]+w[1][2]*t[2]+w[1][3]*t[3]+w[1][4]*t[4]+w[1][5]*t[5]
      r[2]=r[2]/rad
      r[3]=r[3]*a0/ath
      x1=r[3]*Math.cos(r[2])
      x2=x1*Math.sin(r[1])
      x1=x1*Math.cos(r[1])
      x3=r[3]*Math.sin(r[2])
      pw=(p1+p2*t[2]+p3*t[3]+p4*t[4]+p5*t[5])*t[2]
      qw=(q1+q2*t[2]+q3*t[3]+q4*t[4]+q5*t[5])*t[2]
      ra=2.*Math.sqrt(1-pw*pw-qw*qw)
      pwqw=2.*pw*qw
      pw2=1-2.*pw*pw
      qw2=1-2.*qw*qw
      pw=pw*ra
      qw=qw*ra
      r[1]=pw2*x1+pwqw*x2+pw*x3
      r[2]=pwqw*x1+qw2*x2-qw*x3
      r[3]=-pw*x1+qw*x2+(pw2+qw2-1)*x3
      
      return r;
}