...
the original Fortran code to make it easier to follow.  It
is exactly equivelent to the original Fortran and produces
...
function ELP82B(tjj) {
    ...
    let r = [0, 0, 0];
    for (let i = 0; i < 3; i++) {
        mainproblem(elp82data[i], i + 1, d, lp, l, f,r);
    }

    for (let i = 3; i < 9; i++) {
        tides(elp82data[i], t, i + 1, d, lp, l, f, r);
    }

    for (let i = 9; i < 21; i++) {
        planetary(elp82data[i], t, i + 1, d, lp, l, f, r);
    }
...
//Main problem.
function mainproblem(coeficients, ific, d, lp, l, f, r) {
    const rad = 648000. / Math.PI;
...
function planetary(coeficients, t, ific, d, lp, l, f, r) {
...
function tides(coeficients, t, ific, d, lp, l, f, r) {
    const iv = ((ific - 1) % 3)
